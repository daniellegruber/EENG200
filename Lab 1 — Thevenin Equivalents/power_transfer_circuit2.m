...
% Solve for V_T
syms V_a V_b i_1 i_2 V_T
eqn = cell(1,5);
eqn{1} = V_a == V_s - i_1 * R_1;
eqn{2} = V_a == i_1 * R_2;
eqn{3} = V_b == V_s - i_2 * R_3;
eqn{4} = V_b == i_2 * R_5;
eqn{5} = V_T == V_a - V_b;

sol = solve(eqn, [V_a, V_b, i_1, i_2, V_T]);
V_T = sol.V_T;
disp(['V_T = ', num2str(double(V_T))])

% Find R_T
R_T = (R_1*R_2)/(R_1 + R_2) + (R_3*R_5)/(R_3 + R_5);
...
export_fig([save_dir, filesep, 'Power Transfer Circuit 2'], '-png')
...
V_L_exp(1) = 248.2 * 10^-3;
V_L_exp(2) = 197.1 * 10^-3;
V_L_exp(3) = 348.5 * 10^-3;

I_L_exp(1) = 0.823 * 10^-3;
I_L_exp(2) = 1.000 * 10^-3;
I_L_exp(3) = 0.400 * 10^-3;

P_L_exp(1) = 0.203 * 10^-3;
P_L_exp(2) = 0.197 * 10^-3;
P_L_exp(3) = 0.138 * 10^-3;
